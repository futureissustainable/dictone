...
// Fetch synonyms (words with similar meaning)
export async function fetchSynonyms(word: string, maxResults = 30): Promise<RhymeSuggestion[]> {
  try {
    const response = await fetch(
      `${DATAMUSE_API}/words?ml=${encodeURIComponent(word)}&max=${maxResults}&md=s`
    );
    const words: DatamuseWord[] = await response.json();
...
// Fetch all rhymes with scores (for the modal view)
export async function fetchAllRhymesWithScores(word: string, maxResults = 100): Promise<RhymeSuggestion[]> {
  try {
    // Get perfect rhymes
    const rhymeResponse = await fetch(
      `${DATAMUSE_API}/words?rel_rhy=${encodeURIComponent(word)}&max=${maxResults}&md=s`
    ...
    // Get near rhymes
    const nearRhymeResponse = await fetch(
      `${DATAMUSE_API}/words?rel_nry=${encodeURIComponent(word)}&max=${maxResults}&md=s`
    );
    const nearRhymes: DatamuseWord[] = await nearRhymeResponse.json();

    // Combine and deduplicate, marking perfect vs near
    const seen = new Set<string>();
    const all: RhymeSuggestion[] = [];

    // Perfect rhymes get higher base scores
    ...
    // Near rhymes
    for (const item of nearRhymes) {
      if (!seen.has(item.word.toLowerCase())) {
        seen.add(item.word.toLowerCase());
        all.push({
          word: item.word,
          score: Math.max(0, (item.score || 0) - 500), // Reduce score for near rhymes
          numSyllables: item.numSyllables,
          tags: item.tags,
        });
      }
